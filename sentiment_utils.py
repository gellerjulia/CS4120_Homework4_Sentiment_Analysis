...
def get_prfa(dev_y: list, preds: list, verbose=False) -> tuple:
    ...
    """
    prc = precision_score(dev_y, preds)
    re = recall_score(dev_y, preds)
    f1 = f1_score(dev_y, preds)
    acc = accuracy_score(dev_y, preds)
    if verbose:
        print(prc, re, f1, acc)
    return (prc, re, f1, acc)
...
def create_training_graph(metrics_fun: Callable, train_feats: list, dev_feats: list, kind: str, savepath: str = None, verbose: bool = False) -> None:
    ...
    # create percent split of trianing data to use
    pct_training_data = [i for i in np.arange(0.2, 1.2, 0.2)]
    # split training data into above percent splits
    X_train_splits = [X_train[0:math.ceil(len(X_train)*pct)] for pct in pct_training_data]
    y_train_splits = [y_train[0:math.ceil(len(X_train)*pct)] for pct in pct_training_data]
    ...
    for x_split_train,y_split_train in zip(X_train_splits, y_train_splits):
        if verbose:
            print(pct_training_data[i], 'of training data used')
        # get model metrics
        metrics = metrics_fun(x_split_train, y_split_train, X_dev, y_dev, verbose=verbose)
        # save y's to approriate graph y list
        y1.append(metrics[0]) 
        y2.append(metrics[1])
        y3.append(metrics[2])
        y4.append(metrics[3])
    ...
    # plot graph
    plt.plot(pct_training_data, y1, label = "precision") 
    plt.plot(pct_training_data, y2, label = "recall") 
    plt.plot(pct_training_data, y3, label = "f1 score") 
    plt.plot(pct_training_data, y4, label = "accuracy") 
...
def featurize_own(vocab: list, data_to_be_featurized_X: list, binary: bool = False, verbose: bool = False) -> list:
    ...
    # initalize count of datapoints already featurized 
    cnt = 0
    if verbose:
    ...
    for rev in data_to_be_featurized_X:
        # get the count of each word in the vocab in the current review 
        x = [rev.count(word) for word in vocab]
        if binary:
...
def pct_zeros_help(vect):
     """
    Helper function for pct_zeros that calcualtes the percent of entries that are 0 in the given vector.
    Args:
...
def pct_zeros(X):
    ...
    Returns:
        float
  """
    pcts = [pct_zeros_help(x) for x in X]
